import com.shaoyx.mybatis.mapper.CacheMapper;
import com.shaoyx.mybatis.pojo.Emp;
import com.shaoyx.mybatis.utils.SqlSessionUtil;
import org.apache.ibatis.io.Resources;
import org.apache.ibatis.session.SqlSession;
import org.apache.ibatis.session.SqlSessionFactory;
import org.apache.ibatis.session.SqlSessionFactoryBuilder;
import org.junit.Test;

import java.io.IOException;
import java.io.InputStream;


/**
 * Date:2023/1/27
 *
 * @author SYX
 */
public class CacheMapperTest {

    @Test
    public void selectOneByEmpId() {
        SqlSession sqlSession = SqlSessionUtil.getSqlSession();
        CacheMapper mapper = sqlSession.getMapper(CacheMapper.class);
        Emp emp = mapper.selectOneByEmpId(1);
        System.out.println(emp);

        mapper.updateAgeByEmpId(21, 1);

        Emp emp2 = mapper.selectOneByEmpId(1);
        System.out.println(emp2);


        SqlSession sqlSession2 = SqlSessionUtil.getSqlSession();
        CacheMapper mapper2 = sqlSession2.getMapper(CacheMapper.class);
        Emp emp3 = mapper.selectOneByEmpId(1);
        System.out.println(emp3);

    }

    @Test
    public void firstCache() {
        SqlSession sqlSession = SqlSessionUtil.getSqlSession();
        CacheMapper mapper = sqlSession.getMapper(CacheMapper.class);
        Emp emp = mapper.selectOneByEmpId(1);
        System.out.println(emp);

        sqlSession.clearCache();

        Emp emp2 = mapper.selectOneByEmpId(1);
        System.out.println(emp2);


        SqlSession sqlSession2 = SqlSessionUtil.getSqlSession();
        CacheMapper mapper2 = sqlSession2.getMapper(CacheMapper.class);
        Emp emp3 = mapper.selectOneByEmpId(1);
        System.out.println(emp3);

    }

    @Test
    public void testCache() throws IOException {
        InputStream is = Resources.getResourceAsStream("mybatis-config.xml");

        SqlSessionFactory sqlSessionFactory = new SqlSessionFactoryBuilder().build(is);
        SqlSession sqlSession1 = sqlSessionFactory.openSession(true);

        CacheMapper mapper1 = sqlSession1.getMapper(CacheMapper.class);
        Emp emp1 = mapper1.selectOneByEmpId(1);
        System.out.println(emp1);
        sqlSession1.close();

        SqlSession sqlSession2 = sqlSessionFactory.openSession(true);
        CacheMapper mapper2 = sqlSession2.getMapper(CacheMapper.class);
        Emp emp2 = mapper2.selectOneByEmpId(1);
        System.out.println(emp2);
        sqlSession2.close();



    }

}
